...
class GitOpsScaffold:
    ...
    async def scaffold_service(
        ...
        """Create managed service values.yaml in haven-gitops."""
        if not self._is_configured():
            logger.info("GitOps not configured — skipping scaffold_service(%s/%s)", tenant_slug, service_name)
            return
        try:
            ...
            logger.info("Scaffolded service '%s/%s' in haven-gitops", tenant_slug, service_name)
        except Exception as exc:
            logger.error("scaffold_service(%s/%s) failed: %s", tenant_slug, service_name, exc)

    async def delete_service(self, tenant_slug: str, service_name: str) -> None:
        """Delete managed service file from haven-gitops."""
        if not self._is_configured():
            logger.info("GitOps not configured — skipping delete_service(%s/%s)", tenant_slug, service_name)
            return
        try:
            ...
            logger.info("Deleted service '%s/%s' from haven-gitops", tenant_slug, service_name)
        except Exception as exc:
            logger.error("delete_service(%s/%s) failed: %s", tenant_slug, service_name, exc)
