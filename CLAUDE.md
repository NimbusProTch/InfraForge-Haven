...
- **cert-manager Cloudflare DNS-01 cleanup race**: Fresh cluster bootstrap sonrası `iyziops-wildcard` cert `Issuing`'de takılabilir — cleanup `DELETE "/zones//dns_records/<id>"` (boş zone ID) fail eder, stale `_acme-challenge.iyziops.com` TXT'leri birikir. Geçici fix: manual `curl DELETE` + cert sil + cert-manager restart. **Kalıcı fix**: Cloudflare API token'a `User:Read` scope ekle (halihazırda `Zone:Read + DNS:Edit` var).
