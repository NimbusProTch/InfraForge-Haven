...
export default function GdprTab({ tenantSlug, accessToken }: GdprTabProps) {
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-zinc-500">GDPR / AVG compliance controls for data portability, erasure, and retention.</p>
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-600">Article 20 — Right to data portability</p>
          </div>
        </div>
        <p className="text-xs text-zinc-500 mb-3">Download all tenant data including applications, deployments, members, and configurations.</p>
        <button
          onClick={exportData}
          disabled={exporting}
      ...
      <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            <h3 className="text-sm font-medium text-zinc-200">Data Retention</h3>
            <p className="text-xs text-zinc-600">Configure how long data is retained</p>
          </div>
        </div>
        <div className="space-y-3">
          {[
            ...
            <div key={label}>
              <label className="block text-xs font-medium text-zinc-400 mb-1.5">
                {label}: <span className="text-zinc-200">{value} days</span>
              </label>
              <input
                type="range"
                min={7}
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-xs font-medium text-zinc-300 transition-colors disabled:opacity-50"
          >
      ...
      {consents.length > 0 && (
        <div className="bg-zinc-900/50 border border-zinc-800 rounded-xl p-5">
          <h3 className="text-sm font-medium text-zinc-200 mb-3">Active Consents</h3>
          <div className="space-y-2">
            {consents.map((c) => (
              <div key={c.id} className="flex items-center justify-between py-2 border-b border-zinc-800/50 last:border-0">
                <div>
                  <p className="text-xs font-medium text-zinc-300">{c.consent_type}</p>
                  <p className="text-xs text-zinc-600">Granted {new Date(c.granted_at).toLocaleDateString()}</p>
                </div>
      ...
      <div className="bg-red-950/20 border border-red-900/30 rounded-xl p-5">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-600">Article 17 — Permanently delete all data</p>
          </div>
        </div>
      ...
      {showErase && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-zinc-900 border border-red-900/50 rounded-xl w-full max-w-md mx-4 shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-4 h-4 text-red-500" />
                <h2 className="text-sm font-semibold text-red-400">Confirm Data Erasure</h2>
              </div>
              <button onClick={() => setShowErase(false)} className="text-zinc-600 hover:text-zinc-300 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="p-5 space-y-4">
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-lg border border-red-900/50 bg-zinc-800 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-red-600 focus:ring-1 focus:ring-red-600/30 font-mono"
                autoFocus
              />
              <div className="flex justify-end gap-2">
                <button onClick={() => { setShowErase(false); setEraseConfirm(""); }} className="px-3 py-1.5 rounded-lg text-xs font-medium text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800 transition-colors">
                  Cancel
                </button>
                <button
                  onClick={eraseData}
