    ...
    <div>
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-md mx-4 shadow-2xl overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
              <div className="flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-zinc-100">Invite Team Member</h2>
              </div>
              <button onClick={() => setShowInvite(false)} className="text-zinc-600 hover:text-zinc-300 transition-colors">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-800 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-blue-600 focus:ring-1 focus:ring-blue-600/30"
                  autoFocus
                />
              </div>

              <div>
                <label className="block text-xs font-medium text-zinc-400 mb-1.5">Display name (optional)</label>
                <input
                  type="text"
      ...
      {members.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-zinc-800 rounded-xl">
          ...
          <p className="text-xs text-zinc-600 mt-1">Invite your first team member to collaborate.</p>
        </div>
      ) : (
            ...
            return (
              <div
                ...
                className={`flex items-center gap-4 px-4 py-3.5 hover:bg-zinc-800/30 transition-colors ${
                  idx < members.length - 1 ? "border-b border-zinc-800/50" : ""
                }`}
              >
                {/* Avatar */}
                  ...
                  {changingRole === member.user_id ? (
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="appearance-none bg-zinc-800 border border-zinc-700 rounded-lg px-3 py-1.5 pr-7 text-xs font-medium text-zinc-300 cursor-pointer hover:border-zinc-600 focus:outline-none focus:border-blue-600 transition-colors"
                      >
                        {ROLES.map((r) => (
                ...
                <button
                  ...
                  className="text-zinc-700 hover:text-red-400 transition-colors disabled:opacity-50 shrink-0"
                  title="Remove member"
                >
