    ...
    <AppShell userEmail={session?.user?.email}>
      <div className="p-6 max-w-5xl">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="text-xs text-zinc-500 mb-1 uppercase tracking-wider">Platform</p>
        ...
        {tenants.length > 0 && (
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-zinc-900/50 border border-zinc-800 rounded-lg text-sm text-zinc-200 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-700 focus:ring-1 focus:ring-zinc-700 transition-colors"
            />
          </div>
        )}
            ...
            {filtered.map((tenant, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <div className="hidden sm:flex items-center gap-3 text-xs text-zinc-600 font-mono">
                    <span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-500">{tenant.cpu_limit} CPU</span>
                    <span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-500">{tenant.memory_limit}</span>
                    <span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-500">{tenant.storage_limit}</span>
                  </div>
