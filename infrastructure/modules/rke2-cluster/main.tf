...
# ---------------------------------------------------------------------------
#  Gateway API CRDs — fetched at runtime by master-cloud-init runcmd
# ---------------------------------------------------------------------------
#  We do NOT embed the experimental-install.yaml bundle here because it is
#  ~600KB (well over Hetzner's 32KB cloud-init user_data limit). Instead,
#  master-cloud-init.yaml.tpl runcmd uses curl to fetch the upstream bundle
#  at the pinned tag right after rke2-server starts, then `kubectl apply
#  --server-side` the file before cilium-operator does its one-shot CRD
#  readiness check.
#
#  Why not let ArgoCD install the CRDs?
#    Cilium operator only checks for CRDs once at startup. If they are not
#    yet present it logs "Required GatewayAPI resources are not found" and
#    never re-checks. By the time ArgoCD has bootstrapped and synced the
#    gateway-api-crds Application, Cilium operator has already given up
#    and Gateway resources stay PROGRAMMED=Unknown until somebody manually
#    `kubectl rollout restart deploy/cilium-operator`.
#
#  Why not a HelmChart resource?
#    Same reason as Hetzner CCM — helm-install Job pods can't tolerate the
#    uninitialized taint that kubelet sets when --cloud-provider=external
#    is on, so the install pod cannot schedule until CCM has run.
#
#  Pin: var.gateway_api_version (default v1.2.0) experimental channel
#  (includes TLSRoute / GRPCRoute / TCPRoute / UDPRoute, which Cilium's
#  optional GVK list looks for).
# ---------------------------------------------------------------------------
